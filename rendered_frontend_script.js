...
function wrapText(context, layer, x, y, maxWidth, letterHeight, lineHeight) {
  ...
  lines.reverse().forEach(function(line,index) {
    if(context.measureText(line).width > maxWidth) {
      ...
      wrapText(context, layer, x, y, maxWidth, letterHeight ,lineHeight);

    } else {
      context.fillText(line, x, y - ( (lineHeight + letterHeight) * index) );
    }
    

  })

  var fullLineHeight = (lineHeight * lines.length) - lineHeight;

  var fullLetterHeight = (letterHeight * lines.length);

  return  fullLineHeight + fullLetterHeight;
...
// noise value
var noise = 20;
function processSepia(canvas,ctx) {
