...
class BladeOne
{
    ...
    public function run($view, $variables = [])
    {
        ...
        $WP_Modular_Post_Content = $this->runInternal($view, $variables, $forced, true, $runFast);
    ...
    public function getTemplateFile($templateName = '')
    {
        ...
        if ($c == 1) {
            ...
            return $this->locateTemplate($this->filePrefix . $templateName . $this->fileExtension);
        } else {
            $file = $arr[$c - 1];
            \array_splice($arr, $c - 1, $c - 1); // delete the last element
            $path = \implode('/', $arr);
            return $this->locateTemplate($path . '/' . $this->filePrefix . $file . $this->fileExtension);
        }
    }
